...
app.post("/api/appointment", async (req, res) => {
  ...
  // Confirm button URL — owner click karega toh patient ko email jayegi
  ...
  try {
    // ── Email to OWNER with Confirm button ──
    await transporter.sendMail({
      from: `"Backbone Physiotherapy" <${process.env.SENDER_EMAIL}>`,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 620px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 10px; overflow: hidden;">
          <div style="background: #1a3a6b; padding: 24px; text-align: clinic;">
          ...
          <div style="padding: 28px;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr style="border-bottom: 1px solid #f0f0f0;">
                ...
                <td style="padding: 12px 0; color: #222; font-size: 14px;">
                  <a href="tel:${phone}" style="color: #1a3a6b; font-weight: bold;">${phone}</a>
                </td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
          ...
          <div style="padding: 0 28px 28px;">
            <div style="background: #f0f7ff; border-radius: 10px; padding: 20px; text-align: clinic; border: 2px dashed #1a3a6b;">
              ...
              <a href="${confirmURL}"
                style="display: inline-block; background: #1a3a6b; color: white; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 15px; letter-spacing: 0.5px;">
                ✅ Confirm Appointment
              </a>
            </div>
          </div>
          ...
          <div style="background: #f8f9fa; padding: 14px 28px; border-top: 1px solid #e0e0e0; text-align: clinic;">
...
// ── CONFIRM APPOINTMENT route (owner clicks button in email) ──
app.get("/api/confirm-appointment", async (req, res) => {
  const { name, email, phone, date, time, service } = req.query;

  if (!email || !name) {
    return res.send(`
      <div style="font-family: Arial; text-align: clinic; padding: 60px;">
  ...
  try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 10px; overflow: hidden;">
          ...
          <div style="padding: 32px;">
            ...
            <div style="background: #f0f7f0; border-radius: 10px; padding: 20px; margin: 0 0 24px; border-left: 4px solid #16a34a;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td style="padding: 6px 0; color: #666; font-size: 13px; width: 40%;">📅 Date</td>
                  <td style="padding: 6px 0; color: #222; font-weight: bold; font-size: 14px;">${date}</td>
                </tr>
                <tr>
                  <td style="padding: 6px 0; color: #666; font-size: 13px;">⏰ Time</td>
                  <td style="padding: 6px 0; color: #222; font-weight: bold; font-size: 14px;">${time}</td>
                </tr>
                <tr>
                  <td style="padding: 6px 0; color: #666; font-size: 13px;">🏥 Service</td>
                  <td style="padding: 6px 0; color: #1a3a6b; font-weight: bold; font-size: 14px;">${service}</td>
                </tr>
                <tr>
                  <td style="padding: 6px 0; color: #666; font-size: 13px;">📍 Location</td>
                  <td style="padding: 6px 0; color: #222; font-size: 13px;">Rion's Hospital, Sector 110, Gurugram</td>
                </tr>
              </table>
            </div>
            ...
            <p style="color: #555; font-size: 14px; margin: 0;">
              ...
              <a href="tel:+9180536 52525" style="color: #1a3a6b; font-weight: bold;">+91-80536 52525</a>
            </p>
          </div>
    ...
    res.send(`
      <div style="font-family: Arial, sans-serif; text-align: clinic; padding: 80px 20px; background: #f0f7f0; min-height: 100vh;">
        <div style="max-width: 500px; margin: 0 auto; background: white; border-radius: 16px; padding: 48px; box-shadow: 0 4px 24px rgba(0,0,0,0.1);">
          <div style="width: 80px; height: 80px; background: #dcfce7; border-radius: 50%; display: flex; align-items: clinic; justify-content: clinic; margin: 0 auto 24px; font-size: 36px;">✅</div>
    ...
    res.send(`
      <div style="font-family: Arial; text-align: clinic; padding: 60px;">
        <h2 style="color: #e53e3e;">❌ Failed to send confirmation</h2>
        <p>Please call the patient directly at <strong>${phone}</strong></p>
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 10px; overflow: hidden;">
          ...
          <div style="padding: 28px;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 12px 0; color: #666; font-size: 13px; width: 40%;">Name</td>
                <td style="padding: 12px 0; color: #1a3a6b; font-weight: bold; font-size: 14px;">${firstName} ${lastName || ""}</td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 12px 0; color: #666; font-size: 13px;">Email</td>
                <td style="padding: 12px 0; color: #222; font-size: 14px;">
                  <a href="mailto:${email}" style="color: #1a3a6b;">${email}</a>
                </td>
              </tr>
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 12px 0; color: #666; font-size: 13px;">Phone</td>
              ...
              <tr style="border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 12px 0; color: #666; font-size: 13px;">Service</td>
                <td style="padding: 12px 0; color: #222; font-size: 14px;">${service || "Not specified"}</td>
              </tr>
              <tr>
                <td style="padding: 12px 0; color: #666; font-size: 13px; vertical-align: top;">Message</td>
                <td style="padding: 12px 0; color: #222; font-size: 14px; line-height: 1.6;">${message}</td>
              </tr>
            </table>
          </div>
          <div style="background: #f8f9fa; padding: 14px 28px; border-top: 1px solid #e0e0e0; text-align: clinic;">
